...
| 保洁巡查 | `/inspections` | 8 项检查项打分、自动折算百分制得分与等级、**环境卫生量化登记（异味/地面干湿/温湿度/通风/消杀频次）与环境卫生评价**、班次/日期/结论/环境等级筛选、巡查详情、一键转问题上报 |
| 问题上报 | `/issues`、`/issues/:id` | 问题上报（可关联巡查记录）、分类/程度/期限、整改流程流转、整改轨迹时间线、超期预警、追加跟进记录 |

环境卫生评价随巡查一起量化记录；同一公厕的多次记录按巡查时间自动对比，**环境卫生较上一次明显退步时，在巡查记录与公厕台账（列表、详情）中标记「环境退步」**并给出上次/本次分值。
...
| GET | `/inspections` | 巡查记录查询（restroom_id/district/inspector/shift/result/env_grade/日期区间/关键字） |
| POST | `/inspections` | 新增巡查，服务端按检查项自动算分、定级、判定结论；随单提交 `env` 时计算环境卫生评价并与同公厕上一次记录对比 |
...
- **环境卫生量化记录**：巡查时可登记异味等级（无/轻微/明显/强烈刺鼻）、地面干湿（干燥洁净/轻微潮湿/明显积水湿滑/积水污渍）、温度℃、相对湿度%、通风状态（良好/一般/不良）与当日消杀频次（次/日）。六项按权重（异味 30%、地面 20%、通风 15%、消杀 15%、温度 10%、湿度 10%）折算为 0-100 的**环境卫生分**并定级（优秀/良好/合格/不合格）；温湿度落在适宜区间（18-26℃、40%-70%）得满分，越偏离扣分越重，消杀达到 3 次/日得满分。指标可不登记，但登记必须填全。
- **退步标记**：同一公厕的环境记录按巡查时间排序，本次环境分较上一次下降 ≥15 分，或由合格及以上跌入「不合格」档，判定为**明显退步**（`env_regressed=true`），在巡查列表/详情、公厕台账列表与详情中标记，并看板汇总退步公厕数量；公厕详情另给出累计退步次数。
...
`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录（每条均带环境卫生量化记录与评价）、13 条不同整改阶段的问题及其完整整改轨迹；其中一座重点公厕近 3 天的环境卫生逐级退步，便于查看台账退步标记。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。已存在的数据库启动时会幂等补齐新增的环境字段，无需手动删库。
